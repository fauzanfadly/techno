                    ...
                    <v-col md="12">
                        <v-card>
                            <v-container>
                                <v-row>
                                    <v-col md="12">
                                        <p class="text-h5">Manufacture Types</p>
                                    </v-col>
                                </v-row>
                                <v-row>
                                    <v-col md="12">
                                ...
                                <v-row justify="end">
                                    <v-col cols="auto">
                                        <v-btn
                                            color="deep-orange"
                                            @click="() => $router.push({ name: 'admin-manufacture-type-create' })"
                                        >
...
const loadData = async ({ page }) => {
    ...
    await Request.get({
        ...
        errorMessage: `Failed when fetching vendor`
    })
        .then(({ data }) => {
...
const editItem = (id) => {
    router.push({ name: 'admin-manufacture-type-detail', params: { id } });
};
...
const deleteItem = async (id) => {
    openLoading();
    await Request.del({
        url: `/api/manufacture-type/delete/${id}`,
